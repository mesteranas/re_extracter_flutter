    ...
    showDialog(context: context, builder: (BuildContext context){
      return AlertDialog(title: Text(_("about")+" "+App.name),content:Center(child:Column(children: [
        ...
        ,child: Column(children: [
          TextFormField(controller: RECode,decoration: InputDecoration(labelText: _("code")),textInputAction: TextInputAction.next,),
          TextFormField(controller: text,decoration: InputDecoration(labelText: _("text")),textInputAction: TextInputAction.newline,),
          ElevatedButton(onPressed: (){
            results=[];
            try{
              RegExp regexp=RegExp(RECode.text);
              var REResults=regexp.allMatches(text.text);
              for (var result in REResults){
                results.add(result.group(0));
              } 
            } catch(error){
              results.add(_("no matchs"));
            }
            setState(() {
